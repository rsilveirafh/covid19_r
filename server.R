...
server <- function(input, output) {
	...
	output$st_select <- renderUI({
		selectInput("st_select", 
					...
					choices = list(`Centro-Oeste` = 
								   	list("Distrito Federal" = "DF", "Goiás" = "GO", 
								   		 "Mato Grosso" = "MT", "Mato Grosso do Sul" = "MS"),
								   `Nordeste` = 
								   	list("Alagoas" = "AL", "Bahia" = "BA", "Ceará" = "CE",
								   		 "Maranhão" = "MA", "Paraíba" = "PB", "Pernambuco" = "PE", 
								   		 "Piauí" = "PI", "Rio Grande do Norte" = "RN", "Sergipe" = "SE"),
								   `Norte` = 
								   	list("Acre" = "AC", "Amapá" = "AP", "Amazonas" = "AM", "Pará" = "PA",
	...
	output$st_plot <- renderPlotly ({
		...
		if (input$st_scale == "st_linear") {
			st_p <- ggplot(st_graph, aes(x = date, y = value, group = name, colour = name)) +
				...
				scale_colour_manual(name = NULL, 
									values = c("#39CCCC", "#0072B6", "#3D9970")) +
				ggtitle(as.vector(st_filter()$state[1])) +
				theme_minimal() +
				theme(legend.text = element_text(size = 10),
					  axis.title = element_text(size = 14),
					  axis.text = element_text(size = 12),
	...
	# Municípios ----
	output$ct_select_state <- renderUI({
		selectInput("ct_select_state", 
					label = "Selecione o Estado:",
					choices = list(`Centro-Oeste` = 
								   	list("Distrito Federal" = "DF", "Goiás" = "GO", 
								   		 "Mato Grosso" = "MT", "Mato Grosso do Sul" = "MS"),
	...
	output$ct_select_city <- renderUI({
		selectInput("ct_select_city", 
